...
class Jupiter:
    ...
    def setTemperature(self, temperature):
        temperatureFormated = int(round(temperature*10,1)) #Zahl mit Komma zu Zahl ohne Komma, z.B. 32.18 (°C) zu 321 (°C)
        binaryTemperature = int(hex(temperatureFormated)[2:],16).to_bytes(2, 'big') # temperature wird zu hexstring umgewandelt; "x0" wird vom hexstring entfernt; hexstring wird als int interpretiert mit base 16; und wird zu binary umgewandelt
        ...
        if temperatureFormated != int(round(self.readTargetTemperature()*10,1)):
            if self.retrys >= 3:
